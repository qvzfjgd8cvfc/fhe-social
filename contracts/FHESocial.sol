...
contract FHESocial is ZamaEthereumConfig {
    ...
    function getMessages(uint256 _channelId, uint256 _offset, uint256 _limit) external view returns (
        ...
        for (uint256 i = 0; i < count; i++) {
            Message storage m = msgs[_offset + i];
            senders[i] = m.sender;
            contents[i] = m.content;
            isAnonymousFlags[i] = m.isAnonymous;
    ...
    function castVote(
        ...
        // Import the encrypted vote from external input
     ...
     */
    function hasUserVoted(uint256 _channelId, address _user) external view returns (bool) {
        require(_channelId < channelCount, "Invalid channel");
     ...
     * @param _channelId Channel ID
     */
    function getMessageCount(uint256 _channelId) external view returns (uint256) {
        require(_channelId < channelCount, "Invalid channel");
